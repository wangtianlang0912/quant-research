...
通过东方财富API批量获取港股全主板行情数据，
使用项目量化策略（趋势跟随、均值回归、布林带、MACD+RSI、唐奇安通道）进行扫描，
生成买卖信号并输出荐股报告。
"""
...
def fetch_hk_spot_tencent(session):
    """通过腾讯财经接口批量获取港股行情数据"""
    logger.info("开始通过腾讯接口获取港股行情数据...")
    
    # 先获取港股主板代码列表（使用东方财富列表接口，只取代码）
    # 如果东方财富也不行，使用核心蓝筹+热门股列表
    
    # 港股核心标的（按成交额排列的大标的）- 代码格式用腾讯 r_hk00700
    core_codes = [
    ...
    # 增加更多标的 - 从 00001 到 09999 的主板代码（只取最活跃的）
    extended_codes = [
        f'{i:05d}' for i in range(1, 10000)
        # 排除一些明显不活跃的编号区间
    ]
        ...
        try:
            ...
            for line in text.strip().split(';'):
                ...
                try:
                    ...
                    code = parts[2] if len(parts) > 2 else ''  # 如 hk00700
                    name = parts[1] if len(parts) > 1 else ''
                    price = safe_float(parts[3])
                    change_pct = safe_float(parts[32])  # 涨跌幅
                    volume = safe_float(parts[6])  # 成交量(手)
                    amount = safe_float(parts[37])  # 成交额(万)
                    turnover = safe_float(parts[38]) if len(parts) > 38 else 0  # 换手率
                    
                    # 成交额从万转为港元
                    amount_hkd = amount * 10000
    ...
    # 如果核心标的获取成功且数量足够，再尝试获取更多标的
    if len(all_quotes) < 50:
        logger.warning(f"港股行情数据不足({len(all_quotes)}只)，尝试获取更多标的...")
        # 尝试通过更大的范围获取
        big_batch_codes = [f'{i:05d}' for i in range(1, 3000)]
        for i in range(0, len(big_batch_codes), batch_size):
            batch = big_batch_codes[i:i+batch_size]
            tencent_codes = [f'r_hk{c}' for c in batch]
            url = f"http://qt.gtimg.cn/q={','.join(tencent_codes)}"
            try:
                r = session.get(url, timeout=10)
                ...
                for line in text.strip().split(';'):
                    ...
                    try:
                        ...
                        if any(kw in name for kw in ['购', '沽', '牛', '熊', '轮']):
                            continue
                        raw_code = code.replace('hk', '').replace('r_hk', '')
                        if f'hk{raw_code}' in [q.code for q in all_quotes]:
                            continue
    ...
    logger.info(f"港股行情数据: {len(all_quotes)} 只")
    return all_quotes


def fetch_hk_spot_list(session, max_pages=10, page_size=500):
    """批量获取港股主板实时行情列表 - 先尝试东方财富，失败则用腾讯接口"""
    ...
    if success and all_items:
        logger.info(f"东方财富获取成功: {len(all_items)} 条原始数据")
        quotes = parse_hk_quotes(all_items)
        if len(quotes) >= 50:
            return quotes
    
    # 东方财富失败，切换到腾讯接口
    logger.warning("东方财富接口不可用，切换到腾讯接口...")
    return fetch_hk_spot_tencent(session)
    
    logger.info(f"原始数据条数: {len(all_items)}")
    return all_items


def parse_hk_quotes(items):
    """解析港股行情数据"""
    quotes = []
    seen = set()
...
def _fetch_kline_tencent(session, raw_code: str, days: int) -> Optional[List[KlineBar]]:
    ...
    try:
        ...
        if not days_data:
            return None
        bars = []
        for d in days_data[-days:]:  # d = [日期, 开, 收, 高, 低, 成交量]
            if len(d) >= 6:
                bars.append(KlineBar(
...
def fetch_hk_kline(session, code, days=60):
    ...
    # 2. 东方财富接口（用裸 requests 快速失败，不走 Retry 适配器）
...
STRATEGY_MAX_BARS = 60   # VWAP/Breakout 最多需要 60 根


def kline_to_bar(k: KlineBar) -> Bar:
    """将 KlineBar 转为策略层 Bar 对象"""
    return Bar(
        symbol='',
        timestamp=datetime.strptime(k.date, '%Y-%m-%d') if '-' in k.date
...
def kline_to_candle(k: KlineBar) -> dict:
    """将 KlineBar 转为 breakout 策略需要的 dict"""
    return {
        'date': k.date,
        'open': k.open,
        'high': k.high,
...
def run_strategy_scan(
    ...
    """
    以正式策略类扫描单只股票，返回信号列表。
    MACD+RSI 需要递增喂入 K 线（2 步），其余策略一步到位。
    """
    ...
    if strategy_name == 'macd_rsi':
        # MACD+RSI 需增量喂入：第一步初始化 EMA，第二步检测交叉
        strategy = strategy_cls()
        context1 = make_context(bars[:-1], strategy.metadata())
        strategy.on_bar(context1)                       # 初始化 EMA + prev
        context2 = make_context(bars, strategy.metadata())
        return strategy.on_bar(context2)                # 检测金叉/死叉

    if strategy_name == 'vwap':
        # VWAP 滚动窗口：10日窗口 + 15%极端偏离过滤
    ...
    if strategy_name == 'breakout':
        # Qullamaggie Breakout 走 scan_symbol，门槛降至 18 分
    ...
    # 其余策略：一步到位（首次 on_bar 即可输出当前方向信号）
    strategy = strategy_cls()
    context = make_context(bars, strategy.metadata())
    return strategy.on_bar(context)
...
def signal_to_score(sig: Signal, strategy_name: str) -> float:
    """
    将正式策略信号转为评分。
    LONG  → 正分（-1/+1 对应满分），SHORT → 负分，FLAT → 0
    """
...
class StockScanResult:
    ...
    strategy_signals: dict = field(default_factory=dict)  # 各策略原始信号
    ...
    for sname, scls in STRATEGY_CLASSES.items():
        try:
            ...
            for sig in signals:
                ...
                if sig.direction == SignalDirection.LONG:
                    result.buy_signals.append(StrategySignal(
                        sname, 'buy', float(sig.strength), sig.reason))
                elif sig.direction == SignalDirection.SHORT:
                    result.sell_signals.append(StrategySignal(
                        sname, 'sell', float(sig.strength), sig.reason))
...
def scan_market(quotes: List[HKQuote], session, top_klines=200):
    """批量扫描市场 — 并发取K线 + 正式策略扫描"""
    ...
    # P2: 并发获取K线并扫描（10线程）
    max_workers = min(10, len(scan_list))
...
def analyze_market_sentiment(session) -> Dict:
    ...
    if hsi_bars and len(hsi_bars) >= 2:
        ...
        if len(hsi_closes) >= 26:
            rsi = calc_rsi(hsi_closes, 14)
            sentiment['hsi_rsi'] = rsi
            dif, dea, macd_bar = calc_macd(hsi_closes)
            sentiment['hsi_macd'] = '金叉' if macd_bar > 0 and dif > dea else '死叉' if macd_bar < 0 else '中性'
            
            ma5 = calc_ma(hsi_closes, 5)
            ma20 = calc_ma(hsi_closes, 20)
            if hsi_closes[-1] > ma5 > ma20:
                sentiment['hsi_trend'] = '多头'
            elif hsi_closes[-1] < ma5 < ma20:
    ...
    if hstech_bars and len(hstech_bars) >= 2:
        ...
        if len(hstech_closes) >= 26:
            rsi = calc_rsi(hstech_closes, 14)
            sentiment['hstech_rsi'] = rsi
            dif, dea, macd_bar = calc_macd(hstech_closes)
            sentiment['hstech_macd'] = '金叉' if macd_bar > 0 and dif > dea else '死叉' if macd_bar < 0 else '中性'
            
            ma5 = calc_ma(hstech_closes, 5)
            ma20 = calc_ma(hstech_closes, 20)
            if hstech_closes[-1] > ma5 > ma20:
                sentiment['hstech_trend'] = '多头'
            elif hstech_closes[-1] < ma5 < ma20:
...
def generate_report(results: List[StockScanResult], sentiment: Dict, sectors, report_date: str) -> str:
    """生成完整的荐股报告"""
...
def save_report(report: str, results: List[StockScanResult], report_date: str):
    ...
    data = {
        ...
        'all_results': [asdict(r) for r in results[:50]],  # 只存前50详细数据
    }
...
def main():
    ...
    # 2. 获取行情数据（内部已含东方财富→腾讯的切换逻辑）
    session = make_session()
    quotes = fetch_hk_spot_list(session)
    if not quotes:
        logger.error("所有行情接口均不可用，使用降级方案：核心蓝筹股列表")
        # 最终降级：使用预设核心蓝筹列表 + 模拟数据
        fallback_codes = [
            '00700', '09988', '01810', '00941', '03690', '02318', '02628',
            '00398', '02328', '00688', '00883', '00005', '00011', '01299',
            '02269', '01093', '00669', '00960', '02313', '00001', '00002',
        ...
        # 尝试通过腾讯获取这些标的的行情
        quotes = fetch_hk_spot_tencent(session)
        if not quotes:
            logger.error("所有接口完全不可用，无法生成报告")
            return None
    ...
    # 4. 策略扫描
    logger.info("开始策略扫描...")
    results = scan_market(quotes, session, top_klines=200)
    ...
    report = generate_report(results, sentiment, sectors, report_date)
